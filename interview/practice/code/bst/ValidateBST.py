...
Each node has a maximum and a minimum value. We validate that each node is valid and if
we get to the end.
O(n) T as we are traversing every node. O(n)We use space on the call stack
"""
